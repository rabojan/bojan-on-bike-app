...
export default function AmbassadorZnamenitostiPage() {
  ...
  const filtered = znamenitosti.filter((z) => {
    if (activeFilter === "Vse") return true;
    return statusLabel[z.status] === activeFilter;
  });

  const objavljenih = znamenitosti.filter((z) => z.status === "approved").length;
  const cakajo = znamenitosti.filter((z) => z.status === "pending").length;
    ...
    <AmbassadorShell>
      <div className="space-y-8">
        ...
        <section className="grid gap-4 sm:grid-cols-3">
          {[
            { value: loading ? "—" : String(znamenitosti.length), label: "vseh predlaganih" },
          ...
          {loading ? (
            ...
            <div className="py-16 text-center">
              ...
              <p className="mt-4 text-sm text-zinc-600">
                {activeFilter === "Vse" ? "Še nisi predlagal nobene znamenitosti." : "Ni predlogov s tem statusom."}
              </p>
              {activeFilter === "Vse" && (
                <Link href="/ambasador/koticek/znamenitosti/nova"
                ...
                <article key={z.id}
                  ...
                  <div>
                    <div className="flex flex-wrap items-center gap-3">
                      <h3 className="text-lg font-black text-white">{z.ime}</h3>
                      <StatusBadge status={z.status} />
                    </div>
                    <div className="mt-1.5 text-sm text-zinc-500">
                      {[z.tip, z.regija].filter(Boolean).join(" · ")}
                    </div>
                    {z.admin_opomba && (
